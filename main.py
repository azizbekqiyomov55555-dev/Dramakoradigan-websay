...
async def process_link(message: types.Message):
    ...
    try:
        ...
        # Formatlarni tozalash (takrorlanmaslik uchun)
        unique_formats = {}
        for fmt in formats_list:
            h = fmt['height']
            # Bu yerda joylashuv to'g'rilandi
...
async def download_video(callback: types.CallbackQuery):
    ...
    # Avval video va audioni birlashtirishga harakat qilamiz
    ydl_opts = {
        'format': f"{format_id}+bestaudio/best", 
        'outtmpl': filename,        'merge_output_format': 'mp4',
        'quiet': True,
        'no_warnings': True,
    }
    ...
    try:
        ...
        if not os.path.exists(filename):
            # Agar birlasha olmasa, faqat videoni yuklab olamiz
             ydl_opts_single = {
                'format': format_id,
                'outtmpl': filename,
                'quiet': True,
